...
import { authSecret, nodeEnv } from "./config";

const createToken = (data: Record<string, unknown>) => {
  const token = jwt.sign(data, authSecret as string, {
...
export const cookieOptions = {
  ...
  secure: nodeEnv === "production", // Ensures the cookie is only sent over HTTPS in production
