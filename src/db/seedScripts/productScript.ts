...
async function productScript() {
  try {
    // get all Sellers
    const allSellers = await SellerModel.find().select("id");
    const allSellerIds = allSellers.map((a) => a._id.toString());
    ...
    for (const sellerId of allSellerIds) {
      ...
      // generate data
      const products = Array.from({ length: productsCount }, () => {
        const music = selectRandomItemFromArray(musicInfo);
